...
static void startStrokeEngineTask(void *pvParameters) {
    ...
    ESP_LOGI("StrokeEngine", "=== Starting Stroke Engine — travel: %.1f mm, keepout: %.1f mm ===",
             strokingMachine.physicalTravel, strokingMachine.keepoutBoundary);
    ...
    while (isInCorrectState()) {
        ...
        if (lastSetting.stroke != settings.stroke) {
            ...
            ESP_LOGI("StrokeEngine", "Stroke: %.0f%% (%.1f mm)", settings.stroke, newStroke);
        ...
        if (lastSetting.depth != settings.depth) {
            ...
            ESP_LOGI("StrokeEngine", "Depth: %.0f%% (%.1f mm)", settings.depth, newDepth);
        ...
        if (lastSetting.sensation != settings.sensation) {
            ...
            ESP_LOGI("StrokeEngine", "Sensation: %.0f%% (%.1f)", settings.sensation, newSensation);
            Stroker.setSensation(newSensation, false);
            lastSetting.sensation = settings.sensation;
        }

        if (lastSetting.pattern != settings.pattern) {
            ESP_LOGI("StrokeEngine", "Pattern change: %d", (int)settings.pattern);
    ...
    ESP_LOGI("StrokeEngine", "=== Stroke Engine stopped ===");
    Stroker.stopMotion();
