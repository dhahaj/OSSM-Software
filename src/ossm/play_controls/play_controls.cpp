...
static void drawPlayControlsTask(void *pvParameters) {
    ...
    unsigned long serialLastUpdated = 0;
    ...
    while (isInCorrectState()) {
        ...
        // Serial telemetry every 5 seconds (substitutes for OLED display)
        if (millis() - serialLastUpdated > 5000) {
            serialLastUpdated = millis();

            static const char *controlNames[] = {"STROKE", "DEPTH", "SENSATION", "BUFFER"};
            const char *modeName = isStrokeEngine
                ? ui::strings::strokeEngineNames[(int)settings.pattern]
                : (isStreaming ? ui::strings::streaming : ui::strings::simplePenetration);

            ESP_LOGI("Status", "[%s] Spd:%.0f%% Str:%.0f%% Dep:%.0f%% Sen:%.0f%% Knob:%s Strokes:%d",
                     modeName, settings.speed, settings.stroke,
                     settings.depth, settings.sensation,
                     controlNames[(int)session.playControl],
                     session.strokeCount);

            if (!isStrokeEngine && !isStreaming) {
                unsigned long elapsed = millis() - session.startTime;
                ESP_LOGI("Status", "  Distance: %.2f m  Time: %lu:%02lu",
                         session.distanceMeters, elapsed / 60000, (elapsed / 1000) % 60);
            }
        }
